...
class ImgTagTransformPass extends BasePass
{
    ...
    protected function getLayout($el) {
        return 'fixed';
        //return ( $el->hasAttr('width') || $el->hasAttr('height') ) ? 'fixed' : 'responsive';
    ...
    protected function setResponsiveImgHeightAndWidth(DOMQuery $el)
    {
        ...
        if ($dimensions !== false) {
            ...
            if($el->hasAttr('width') && $el->attr('height')) {
                return true;
            } else if($el->hasAttr('width') && $el->attr('width')) {
                $proportions = $dimensions['width'] / $el->attr('width');
                $el->attr('height', $dimensions['height'] / $proportions);
            } else if($el->hasAttr('height') && $el->attr('height')) {
                $proportions = $dimensions['height'] / $el->attr('height');
                $el->attr('width', $dimensions['width'] / $proportions);
            } else {
                $el->attr('width', $dimensions['width']);
                $el->attr('height', $dimensions['height']);

                $class = $el->attr('class');
                $is_set_size = $this->setCssSize($el, $dimensions, $class);
                if(!$is_set_size) {
                    $class = $el->get(0)->parentNode->getAttribute('class');
                    $this->setCssSize($el, $dimensions, $class);
                }
            }
    ...
    private function setCssSize($el, $dimensions, $class)
    {
        if (!empty($this->options['css_styles']) && $class) {
            preg_match_all('/\s*([^\s]+)/', $class, $class_names);
            $element_id = $el->attr('id');

            if($this->setSizeImgFromCss('#'.$element_id, $dimensions))
                return true;

            foreach ($class_names[1] as $selector)
                if($this->setSizeImgFromCss('.'.$selector, $dimensions))
                    return true;
        }
    ...
    private function setSizeImgFromCss($selector, $dimensions)
    {
        $preg_match_res = preg_match_all('/'.preg_quote($selector).'\s*(>?\s*img?\s*)?{([^.]+)}/', $this->options['css_styles'], $properties);

        if ($preg_match_res) {
            $preg_match_res = preg_match_all('/(max-width|max-height|width|height):\s*(\d+)px/', $properties[2][0], $sizes);

            if($preg_match_res) {
                if (strrpos($sizes[1][0], 'width') > -1) {
                    $el->attr('width', $sizes[2][0]);
                    $proportions = $dimensions['width'] / $el->attr('width');
                    $el->attr('height', $dimensions['height'] / $proportions);
                    if ($sizes[1][0] == 'width')
                        return true;
                } else if (strrpos($sizes[1][0], 'height') > -1) {
                    $el->attr('height', $sizes[2][0]);
                    $proportions = $dimensions['height'] / $el->attr('height');
                    $el->attr('width', $dimensions['width'] / $proportions);
                    if ($sizes[1][0] == 'height')
                        return true;
